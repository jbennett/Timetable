...
@IBDesignable class FloatLabelTextView: UITextView {
  ...
  private func setup() {
    ...
    if !isIB {
      let nc = NSNotificationCenter.defaultCenter()
      let callback = #selector(layoutSubviews)
      nc.addObserver(self, selector:callback, name:UITextViewTextDidChangeNotification, object:self)
      nc.addObserver(self, selector:callback, name:UITextViewTextDidBeginEditingNotification, object:self)
      nc.addObserver(self, selector:callback, name:UITextViewTextDidEndEditingNotification, object:self)
    }
  }
